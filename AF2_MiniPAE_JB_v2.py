...
#     motif_length, motif_sequence, motif_start, motif_stop, motif_mean_pae,
#     pae_list, formatted_sequence
...
def _safe_motif_len(val):
    """Convert motif_length field to an int; return a large number when NA."""
    try:
        return int(val)
    except Exception:
...
def analyze_pae_direction(pae, chain1_start, chain1_end, chain2_start, chain2_end, 
    ...
    if sequence and len(sequence) == len(pae_min_per_residue):
        ...
        formatted_sequence = "NA"
    
    # Find the longest high-confidence motif that meets length threshold
    motif_sequence = motif_start = motif_stop = motif_length = "NA"
    motif_mean_pae = "NA"
    ...
    if matches:
        ...
        if valid_matches:
            ...
            # Compute mean PAE across the motif region (inclusive)
            try:
                start_idx = int(motif_start) - 1
                stop_idx = int(motif_stop)
                segment = pae_min_per_residue[start_idx:stop_idx]
                if segment:
                    motif_mean_pae = float(np.mean(segment))
            except Exception:
                motif_mean_pae = "NA"
...
def read_scores_from_unzipped_dir(dir_path, pae_threshold, motif_length_threshold, verbose=False,
    ...
    for json_path in json_candidates:
        ...
        best_result["rank"] = rank_id
        best_result["alternative_min_pae"] = alternative_result.get("overall_min_pae", "NA")
        best_result["both_directions_analyzed"] = True
        best_result["selection_strategy"] = "shortest_motif_then_min_miniPAE"
...
def process_files(file_paths, pae_threshold, motif_length_threshold, output_file, verbose=False,
    ...
    with open(output_file, "w") as csvfile:
        ...
        csvfile.write(
            ...
            "motif_sequence,motif_start,motif_stop,motif_mean_pae,pae_list,formatted_sequence\n"
        )
        ...
        for path in file_paths:
            ...
            try:
                ...
                for result in all_ranks_results:
                    ...
                    alt_pae = result.get("alternative_min_pae", "NA")
                    ...
                    csvfile.write(
                        ...
                        f"{result['motif_stop']}," +
                        f"{result.get('motif_mean_pae','NA')}," +
                        ";".join(map(str, pae_list)) + "," +
                        f"{result['formatted_sequence']}\n"
                    )
                    
                    total_ranks_processed += 1
                    _mm = result.get('motif_mean_pae', 'NA')
                    _mm_str = f"{_mm:.2f}" if isinstance(_mm, (int, float)) else str(_mm)
                    print(
                        f"  ✓ {experiment_name} rank_{result['rank']}: {result['direction']}, "
                        f"miniPAE={mini_pae:.2f}, motif_len={result['motif_length']}, motif_mean_pae={_mm_str}"
                    )
